...
def find_code_in_dyffusion(object_name):
    ...
    module = parts[i]
    while i < len(parts) and not os.path.isfile(os.path.join(SRC_PATH, f"{module}.py")):
        i += 1
        if i < len(parts):
            module = os.path.join(module, parts[i])
    if i >= len(parts):
        raise ValueError(f"`object_name` should begin with the name of a module of dyffusion but got {object_name}.")

    with open(os.path.join(SRC_PATH, f"{module}.py"), "r", encoding="utf-8", newline="\n") as f:
        lines = f.readlines()
...
_re_copy_warning = re.compile(r"^(\s*)#\s*Copied from\s+dyffusion\.(\S+\.\S+)\s*($|\S.*$)")
...
def is_copy_consistent(filename, overwrite=False):
    ...
    while line_index < len(lines):
        ...
        theoretical_code = find_code_in_dyffusion(object_name)
...
def check_copies(overwrite: bool = False):
    all_files = glob.glob(os.path.join(SRC_PATH, "**/*.py"), recursive=True)
    diffs = []
